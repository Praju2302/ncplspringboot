...
stage("Jar Publish") {
  ...
  stage('Build Docker Image') {
    ...
    stage('Pushing to ECR') {
     steps{  
         script {
                sh 'aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin 251254236023.dkr.ecr.us-east-2.amazonaws.com'
                sh 'docker tag myrepo:latest 251254236023.dkr.ecr.us-east-2.amazonaws.com/myrepo:latest'
                sh 'docker push 251254236023.dkr.ecr.us-east-2.amazonaws.com/myrepo:latest'
         }
        }
      }
